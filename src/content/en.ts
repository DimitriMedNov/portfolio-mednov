  ...
  systems: {
    ...
    cases: [
      ...
      },
      {
        ...
        desc: "A complete survey platform, with its backend, its web panel and its mobile field-capture app. What holds it together is that a survey is not programmed but defined: questions live in a catalogue of their own and get reused across forms, each form decides which ones are required and which only show up once another was answered a certain way, and both the panel and the app draw the form from that definition, so a new survey is added to the database rather than coded. Answers are stored with the location they were taken from, which is what lets a result be read by area and not only as a total. On the backend the queries are generated with sqlc from the SQL, so a schema change breaks the build instead of failing in production, and the migrations ship inside the binary and run on start.",
